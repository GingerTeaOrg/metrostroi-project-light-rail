if not UF and Metrostroi then
	...
	timer.Simple(0.05, function()
		for name in pairs(scripted_ents.GetList()) do
			...
			if string.sub(name, 1, #prefix) == prefix and scripted_ents.Get(name).Base == "gmod_subway_base" and not scripted_ents.Get(name).NoTrain or string.sub(name, 1, #prefix2)
						== prefix2 and scripted_ents.Get(name).Base == "gmod_subway_base" and not scripted_ents.Get(name).NoTrain or string.sub(name, 1, #prefix2) == prefix2
						and scripted_ents.Get(name).Base == "gmod_subway_uf_base" and not scripted_ents.Get(name).NoTrain or string.sub(name, 1, #prefix) == prefix and scripted_ents.Get(name).Base
						== "gmod_subway_uf_base" and not scripted_ents.Get(name).NoTrain then
...
timer.Create("RBLHousekeeping", 30, 0, function()
	...
	for k, v in pairs(UF.IBISRegisteredTrains) do if not IsValid(k) then k = nil end end
end)
...
function UF.RegisterTrain(LineCourse, train) -- Registers a train for the RBL simulation
	...
	elseif (LineCourse ~= "0000" or LineCourse ~= "00000") and #LineCourse > 3 and UF.IBISRegisteredTrains[train] == LineCourse then
		...
		output = true
		return output
	elseif LineCourse ~= "0000" and LineTable[string.sub(LineCourse, 1, 2)] and #LineCourse >= 4 and UF.IBISRegisteredTrains[train] ~= LineCourse
				and UF.checkDuplicateTrain(train, LineCourse) then
		...
		return output
	elseif LineCourse ~= "0000" and LineTable[string.sub(LineCourse, 1, 2)] and #LineCourse >= 4 and UF.IBISRegisteredTrains[train] ~= LineCourse
				and not UF.checkDuplicateTrain(train, LineCourse) then
		UF.IBISRegisteredTrains[train] = LineCourse
		train.RBLRegistered = true
