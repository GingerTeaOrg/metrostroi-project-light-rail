...
local function switchandsetoperation( ply, op )
	...
	ply:ConCommand( "gmod_toolmode mplr_trafficeditor" )
	ply:GetWeapon( "gmod_tool" ).Mode = "mplr_trafficeditor"
...
local function createtrack( id, track )
	...
	for k, v in pairs( track.Next ) do
		if tracks[ v ] and not table.HasValue( tracks[ v ].Prev, id ) then
	...
	for k, v in pairs( track.Prev ) do
		if tracks[ v ] and not table.HasValue( tracks[ v ].Next, id ) then
	...
	if prevtrack then
		for k, v in pairs( prevtrack.Next ) do
			if tracks[ v ] and not table.HasValue( track.Next, v ) and table.HasValue( tracks[ v ].Prev, id ) then
		...
		for k, v in pairs( prevtrack.Prev ) do
			if tracks[ v ] and not table.HasValue( track.Prev, v ) and table.HasValue( tracks[ v ].Next, id ) then
...
function TOOL:LeftClick( trace )
	...
	if op == 0 then
		...
		elseif op == 3 then
			local traceEnt = IsValid( trace.Entity ) and trace.Entity or nil
			local entClass = traceEnt and traceEnt:GetClass() or nil
			if IsValid( traceEnt ) and ( entClass == "trolleybus_trafficlight" or self.ValidMPLRLights[ entClass ] ) and traceEnt.Data then
				local id = self:GetClientNumber( "trackid" )
				local track = Trolleybus_System.GetTrafficTracks()[ id ]
				if track then
					track.TrafficLight = traceEnt.Data.ID
					savetracks( id )
				end
			else
				return false
...
function TOOL:RightClick( trace )
	...
	if op == 0 then
		if self:GetOwner():KeyDown( IN_USE ) then
			...
			local id = self:GetClientNumber( "trackid" )
			if removetrack( id ) then savetracks( id ) end
		end
	else
		if op == 2 then
...
function TOOL:Reload()
	...
	if op == 0 then
		...
		if Trolleybus_System.GetTrafficTracks()[ id ] then
			self:GetOwner():ConCommand( "mplr_trafficeditor_trackid " .. id )
			net.Start( "TrolleybusTrafficTrackEditor" )
			net.WriteUInt( 1, 4 )
			net.WriteUInt( id, 32 )
			net.Send( self:GetOwner() )
...
net.Receive( "TrolleybusTrafficTrackEditor", function( len, ply )
	--if not Trolleybus_System.RunEvent( "HasAccessToTool", ply, "trolleytrafficeditor" ) then return end
	local self = ply:HasWeapon( "gmod_tool" ) and ply:GetWeapon( "gmod_tool" ):GetMode() == "mplr_trafficeditor" and ply:GetWeapon( "gmod_tool" ):GetToolObject()
	...
	elseif cmd == 3 then
		...
		self:BuildRotation( pos1, pos2, ang, self:GetClientNumber( "rotation_segments" ), self:GetClientNumber( "rotation_curvature" ), R, function( i, count, pos, prevpos )
			...
			if type ~= 0 then t.TurnSignal = type end
