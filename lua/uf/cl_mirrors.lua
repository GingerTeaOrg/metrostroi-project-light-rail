-- Original credit: TrolleyBus Simulator, https://github.com/ShadowBonnieRUS/Garry-s-Mod-Trolleybus-System/blob/master/lua/trolleybus_system/mirrors.lua
...
local MIRROR = {
	...
	SetPos = function( self, pos ) self.m_Pos = Vector( pos ) end,
	...
	SetAngles = function( self, ang ) self.m_Ang = Angle( ang ) end,
	...
	GetParent = function( self ) return self.m_Parent end,
	SetLocalPos = function( self, lpos ) self.m_LPos = lpos end,
	GetLocalPos = function( self ) return Vector( self.m_LPos ) end,
	SetLocalAngles = function( self, lang ) self.m_LAng = lang end,
	GetLocalAngles = function( self ) return Angle( self.m_LAng ) end,
	...
	Draw = function( self )
		if self.DRAWING or not self.Texture then return end
	...
	Update = function( self )
		...
		render.RenderView( {
			x = 0,
			y = 0,
			w = W * MIRROR_TEXTURE_SCALE,
			h = H * MIRROR_TEXTURE_SCALE,
			origin = self:GetPos(),
			angles = ang,
			aspectratio = w / h,
			drawviewmodel = false,
	...
	ShouldDraw = function( self )
		if not self:IsValid() or not self.W or not self.H then return false end
...
hook.Add( "ShouldDrawLocalPlayer", "Trolleybus_System.Mirrors", function( ply ) if DrawLocalPlayer then return DrawLocalPlayer end end )
...
hook.Add( "PostRender", "Trolleybus_System.Mirrors", function()
	...
	for k, v in pairs( Trolleybus_System.Mirrors ) do
		if not k:ShouldDraw() or rendermirrors == 2 and k.Trolleybus ~= bus then continue end
		if not started then
			started = true
...
hook.Add( "PreRender", "Trolleybus_System.Mirrors", function() if ResetingView then return true end end )
