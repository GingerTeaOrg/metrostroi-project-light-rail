...
spawnmenu.AddContentType("entity_lightrail", function(parent, data)
...
spawnmenu.AddCreationTab("Metrostroi: Project Light Rail", function()
    local panel = vgui.Create("SpawnmenuContentPanel")
    return panel
end, "materials/vgui/icon.png", 25)

-- Hook to populate the entity list in the spawn menu
hook.Add("PopulateList", "PopulateList", function(content, tree, node)
    local cats = {{}, {}} -- Initialize categories
    print("POPULATEPOPULATE")
    -- Iterate through scripted entities and categorize them
    for name,_ in pairs(scripted_ents.GetList()) do
		print(name)
        if UF.IsTrainClass(name) then
    ...
    local order = {{cats, tree:Root()}} -- Initialize order of categories
    while #order > 0 do -- Process categories
        ...
        for k, v in pairs(cur[1][2]) do
            ...
            order[#order + 1] = {v, node} -- Add subcategories to the order
        end
    end
