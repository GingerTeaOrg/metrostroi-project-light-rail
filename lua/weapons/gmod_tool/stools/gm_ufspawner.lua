...
local function Trace( ply, tr )
	...
	if tr.Hit then
		-- Setup trace to find out if this is a track
		local tracesetup = {}
...
function TOOL:UpdateGhost()
	...
	for i, e in ipairs( self.GhostEntities ) do
		...
		if i == 1 then
			...
			if self:GetOwner():GetNW2Bool( "mplr_train_spawner_rev" ) then ang = ang + Angle( 0, 180, 0 ) end
...
function TOOL:SpawnWagon( trace )
	...
	for i = 1, self.Settings.WagNum do
		...
		for _, set in ipairs( self.Train.Spawner ) do
			...
			if set[ 3 ] == "List" then
				if set[ 6 ] and type( set[ 6 ] ) == "function" then
					set[ 6 ]( ent, val, LastRot, i, self.Settings.WagNum )
				else
					SetValue( ent, set[ 1 ], val )
				end
			elseif set[ 3 ] == "Boolean" then
				if set[ 5 ] and type( set[ 5 ] ) == "function" then
					set[ 5 ]( ent, val, LastRot, i, self.Settings.WagNum )
				else
					ent:SetNW2Bool( set[ 1 ], val )
				end
			elseif set[ 3 ] == "Slider" then
				if set[ 8 ] and type( set[ 8 ] ) == "function" then
					set[ 8 ]( ent, val, LastRot, i, self.Settings.WagNum )
				else
					ent:SetNW2Int( set[ 1 ], val )
				end
			end
		end
...
function TOOL:Finish()
	if self.GhostEntities and #self.GhostEntities > 0 then
		for _, e in pairs( self.GhostEntities ) do
			SafeRemoveEntity( e )
		end
	end
...
function TOOL:LeftClick( trace )
			...
			if trace.Entity.ClassName == ( self.Train.Spawner.head or self.Train.ClassName ) or trace.Entity.ClassName == self.Train.Spawner.interim then
				...
				for k, ent in ipairs( trace.Entity.WagonList ) do
					...
					for i, set in ipairs( self.Train.Spawner ) do
						...
						if set[ 3 ] == "List" then
							if set[ 6 ] and type( set[ 6 ] ) == "function" then
								set[ 6 ]( ent, val, rot, k, self.Settings.WagNum )
							else
								SetValue( ent, set[ 1 ], val )
							end
						elseif set[ 3 ] == "Boolean" then
							if set[ 5 ] and type( set[ 5 ] ) == "function" then
								set[ 5 ]( ent, val, rot, k, self.Settings.WagNum )
							else
								ent:SetNW2Bool( set[ 1 ], val )
							end
						elseif set[ 3 ] == "Slider" then
							if set[ 8 ] and type( set[ 8 ] ) == "function" then
								set[ 8 ]( ent, val, rot, k, self.Settings.WagNum )
							else
								ent:SetNW2Int( set[ 1 ], val )
							end
						end
	...
	if SERVER then
		if self.Settings.WagNum > C_MaxWagons:GetInt() then self.Settings.WagNum = C_MaxWagons:GetInt() end
		if Metrostroi.TrainCountOnPlayer( self:GetOwner() ) + self.Settings.WagNum > GetConVar( "mplr_maxtrains_onplayer" ):GetInt() * C_MaxWagons:GetInt() or Metrostroi.TrainCount() + self.Settings.WagNum > GetConVar( "mplr_maxtrains" ):GetInt() * C_MaxWagons:GetInt() then
			...
			if trace.Entity.ClassName == ( self.Train.Spawner.head or self.Train.ClassName ) or trace.Entity.ClassName == self.Train.Spawner.interim then
				...
				for k, ent in pairs( trace.Entity.WagonList ) do
					...
					for i, set in ipairs( self.Train.Spawner ) do
						...
						if set[ 3 ] == "List" then
							if set[ 6 ] and type( set[ 6 ] ) == "function" then
								set[ 6 ]( ent, val, rot, k, self.Settings.WagNum, true )
							else
								SetValue( ent, set[ 1 ], val )
							end
						elseif set[ 3 ] == "Boolean" then
							if set[ 5 ] and type( set[ 5 ] ) == "function" then
								set[ 5 ]( ent, val, rot, k, self.Settings.WagNum, true )
							else
								ent:SetNW2Bool( set[ 1 ], val )
							end
						elseif set[ 3 ] == "Slider" then
							if set[ 8 ] and type( set[ 8 ] ) == "function" then
								set[ 8 ]( ent, val, rot, k, self.Settings.WagNum, true )
							else
								ent:SetNW2Int( set[ 1 ], val )
							end
						end
