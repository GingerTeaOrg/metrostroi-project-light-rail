...
local signalTypes2 = file.Find( "models/lilly/mplr/scenery/trackside/signage/*.mdl", "GAME", "nameasc" )
...
function TOOL.BuildCPanel( panel )
	local typeList = table.Merge( signalTypes, signalTypes2 )
	local tool = tool or {}
	local basePath = "models/lilly/uf/signage/"
	local basePath2 = "models/lilly/mplr/scenery/trackside/signage"
	CreateClientConVar( "uf_signs_sign_type", "speed_60" )
	...
	for _, modelPath in pairs( typeList ) do
		local button = vgui.Create( "DButton", scrollPanel )
		button:SetText( modelPath )
		button:Dock( TOP )
		button.DoClick = function()
			if file.Exists( basePath .. modelPath ) then
				...
				LocalPlayer():EmitSound( "buttons/button3.wav" )
				modelPreview:SetModel( basePath2 .. modelPath )
				RunConsoleCommand( "uf_signs_sign_type", basePath2 .. modelPath )
			end
		end
	end
