...
function ENT:Think()
	...
	if table.IsEmpty( MPLR.IBISRegisteredTrains ) and not trainPresent then -- either fall back to idle, train list, or current train display
		self.Mode = 0
		self.IgnoredTrains = {}
	elseif table.IsEmpty( MPLR.IBISRegisteredTrains ) and trainPresent then
		self.Mode = 2
		self.IgnoredTrains = {}
	elseif self.Train1ETA and tonumber( self.Train1ETA ) > 0 and not trainPresent then
		self.Mode = 1
...
function ENT:TrainPresent()
	local function trainTrackingFallback()
		...
		print( myPath )
		local samePath = myPath == trainPath
		if not samePath then return false end
		local positive = trainPos.x < self.TrackPosition.x
		local distance
		if positive then
			distance = self.TrackPosition.x - trainPos.x
		else
			distance = trainPos.x - self.TrackPosition.x
		end

		if distance > 20 then return false end
		if not self.Train1Ent.IBIS or not next( MPLR.IBISRegisteredTrains ) then
			return "DestUnknown"
		elseif self.Train1Ent.IBIS then
			local ibis = self.Train1Ent.IBIS
			local destination = string.lower( ibis.Destination )
			if string.find( destination, "nicht einsteigen" ) or string.find( destination, "leerfahrt" ) or string.find( destination, "betriebsfahrt" ) or string.find( destination, "dienstfahrt" ) or string.find( destination, "leerzug" ) or string.find( destination, "probewagen" ) then
				return "DontBoard"
			else
				return "normal"
			end
	...
	if IsValid( self.PairedPlatform ) then
		print( "paired" )
