...
function ENT:CreateINDUSICoil( ent, rear )
	local coil = ents.Create( "gmod_train_indusi_coil" )
	ent.INDUSICoil = coil
...
function ENT:Initialize()
	...
	self.WipeDown = false
	self.Wiped = false
	self.MUSystemsInitialized = false
end
...
function ENT:Think()
	if not self.MUSystemsInitialized then
		if IsValid( self.SectionB ) and not self.SectionA then
			self.SectionB:LinkMUSystems( self )
		elseif IsValid( self.SectionA ) and IsValid( self.SectionB ) then
			self.SectionB:LinkMUSystems( self )
			self.SectionA:LinkMUSystems( self )
		end

		self.MUSystemsInitialized = true
	end
...
function ENT:GenerateJerks()
	...
	self.PrevAcceleration = self.Acceleration --local rjerk = ((roll - (self.PrevRoll or 0)) / self.DeltaTime)
	self.PrevRoll = roll
	...
	if self.RollJerk ~= rjerkstate then
		...
		if CurTime() - self.PrevTriggerTime1 > 0.01 then --[[ math.abs(self.Speed) > 0.5 and --]]
			--self.PrevTriggerTime1 = CurTime()
			--self.FrontBogey:EmitSound( table.Random( self.SoundNames[ "junk_small" ] ), 69, math.random( 96, 110 ) )
			--self.FrontBogey:EmitSound( table.Random( self.SoundNames[ "junk_medium" ] ), 72, math.random( 96, 110 ) )
			--self.RearBogey:EmitSound( table.Random( self.SoundNames[ "junk_small" ] ), 69, math.random( 96, 110 ) )
