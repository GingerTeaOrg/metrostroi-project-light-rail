...
surface.CreateFont( "Trebuchet24", {
	--Creating BUILTIN font (idk what happened with this font')
	font = "Trebuchet",
	size = 24,
	weight = 400,
	blursize = 0,
...
local C_CameraJerk = GetConVar( "mplr_camera_jerk_intensity" )
local whitelist = {
	[ "CHudChat" ] = true,
...
hook.Add( "HUDShouldDraw", "MPLRHUDHider", function( name ) if LocalPlayer().InMetrostroiTrain and C_DisableHUD:GetBool() and not whitelist[ name ] then return false end end )
--------------------------------------------------------------------------------
-- Buttons layout
--------------------------------------------------------------------------------
...
function ENT:ShouldDrawClientEnt( k, v )
	...
	if v.hideseat then
		...
		if IsValid( seat ) and self ~= seat:GetParent() then return false end
...
function ENT:SpawnCSEnt( k, override )
	...
	if v and not IsValid( self.ClientEnts[ k ] ) and v.model ~= "" then
		...
		if self.Anims[ k ] and self.Anims[ k ].value and type( self.Anims[ k ].value ) == "number" then cent:SetPoseParameter( "position", self.Anims[ k ].value ) end
		if v.bodygroup then
...
hook.Add( "Think", "SpawnElapsed", function()
	elapsed = SysTime()
	spawnedCount = 0
end )
...
function ENT:CreateCSEnts()
	...
	if self.ClientPropsOv then
		for k in pairs( self.ClientPropsOv ) do
			if k ~= "BaseClass" and not IsValid( self.ClientEnts[ k ] ) and self:SpawnCSEnt( k ) then if SysTime() - elapsed > time then return false end end
		end
	end
...
function ENT:GetWagonNumber()
	...
	if number <= 0 then number = self:EntIndex() end
	return number
end
...
function ENT:Initialize()
	...
	if self.Lights then
		for i, lightData in pairs( self.Lights ) do
			if lightData.changable then self.LightsOverride[ i ] = table.Copy( lightData ) end
		end
	end
...
function ENT:UpdateTextures()
	...
	for id, ent in pairs( self.ClientEnts ) do
		...
		for k in pairs( ent:GetMaterials() ) do
...
function ENT:OnRemove( nfinal )
	...
	for _, v in pairs( self.Sounds ) do
		if type( v ) ~= "function" and type( v ) ~= "table" then self:DestroySound( v ) end
	end
	...
	for _, v in pairs( self.PassengerEntsStuck or {} ) do
		SafeRemoveEntity( v )
	end
...
function ENT:Think()
	...
	if self.GlowingLights and ( self.HeadlightShadows ~= C_Shadows1:GetBool() or self.OtherShadows ~= C_Shadows2:GetBool() or self.RedLights ~= C_Shadows3:GetBool() or self.AAEnabled ~= ( C_AA:GetInt() > 1 ) ) then --or self.OtherLights ~= C_Shadows4:GetBool() --[[or self.SpritesEnabled ~= C_Sprites:GetBool())]]
		self.HeadlightShadows = C_Shadows1:GetBool()
		self.OtherShadows = C_Shadows2:GetBool()
		self.RedLights = C_Shadows3:GetBool()
		self.OtherLights = true --C_Shadows4:GetBool() --TODO maybe reimplement these to be independant from upstream Metrostroi
		self.SpritesEnabled = true --C_Sprites:GetBool()
	...
	local disableSeatShadows = true --C_DisableSeatShadows:GetBool() --TODO METROSTROI UPSTREAM
			...
			for kp, pan in pairs( self.ButtonMapMPLR ) do
				...
				if pan.mouse and not pan.outside and pan.aimX and pan.aimY then
					...
					if pan.OldAimX ~= aimX or pan.OldAimY ~= aimY then
						net.Start( "mplr-mouse-move", true )
	...
	if self.ButtonMapMPLR and ( not self.LastCheck or RealTime() - self.LastCheck > 0.5 ) then
		...
		for k, v in pairs( self.ClientProps ) do
			...
			if v.nohide or screenshotMode then
				if not IsValid( cent ) then self:SpawnCSEnt( k, true ) end
				continue
			end
	...
	if self.AutoAnims and self.AutoAnimNames then
		local aAnims = self.AutoAnims
		local aAnimNames = self.AutoAnimNames
		local hidden = self.Hidden
		for i = 1, #aAnims do
			if not aAnimNames[ i ] or not hidden[ aAnimNames[ i ] ] then aAnims[ i ]( self ) end
		end
	end
	...
	if self.RenderClientEnts and self.PassengerEnts then
		local stuck = self.PassengerEntsStuck
		for i, v in ipairs( self.LeftDoorPositions ) do
			if self:GetPackedBool( "DoorLS" .. i ) and not IsValid( stuck[ i ] ) then
			...
			elseif not self:GetPackedBool( "DoorLS" .. i ) and IsValid( stuck[ i ] ) then
				SafeRemoveEntity( stuck[ i ] )
			end
		end

		for i, v in ipairs( self.RightDoorPositions ) do
			if self:GetPackedBool( "DoorRS" .. i ) and not IsValid( stuck[ -i ] ) then
				local ent = ClientsideModel( table.Random( self.PassengerModels ), RENDERGROUP_OPAQUE )
				ent:SetPos( self:LocalToWorld( Vector( v.x, v.y, self:GetStandingArea().z ) ) )
				ent:SetAngles( self:LocalToWorldAngles( Angle( 0, v.y < 0 and -90 or 90, 0 ) ) )
				ent:SetSkin( math.floor( ent:SkinCount() * math.random() ) )
			...
			elseif not self:GetPackedBool( "DoorRS" .. i ) and IsValid( stuck[ -i ] ) then
				SafeRemoveEntity( stuck[ -i ] )
			end
		end
...
function ENT:BlockInput( block )
	if IsValid( LocalPlayer().InMetrostroiTrain ) then if self ~= LocalPlayer().InMetrostroiTrain then block = false end end
...
hook.Add( "Think", "mplr_mouse_handle", function()
...
hook.Add( "PlayerEnteredVehicle", "mplr_mouse_handle", function( ply, veh )
...
local DrawTextHand = function( txt, x, y, col ) draw.SimpleText( txt, "Schedule_Hand", x, y, Color( 0, 15 * col.y, 85 * col.z ), 0, 0 ) end
local DrawTextHandSmall = function( txt, x, y, col ) draw.SimpleText( txt, "Schedule_Hand_Small", x, y, Color( 0, 15 * col.y, 85 * col.z ), 0, 0 ) end
local DrawTextMachine = function( txt, x, y ) draw.SimpleText( txt, "Schedule_Machine", x, y, Color( 0, 0, 0 ), 0, 0 ) end
local DrawTextMachineSmall = function( txt, x, y ) draw.SimpleText( txt, "Schedule_Machine_Small", x, y, Color( 0, 0, 0 ), 0, 0 ) end
...
local Schedule = {
	stations = { { "Station 1", os.time() + 20 }, { "Station 2", os.time() + 46 }, { "Station 3", os.time() + 80 }, { "Station 4", os.time() + 95 }, { "Station 5", os.time() + 120 } },
	total = 2000,
	interval = 300,
...
function ENT:DrawSchedule( panel )
	...
	for i = 1, N do
		Schedule.stations[ i ] = { self:GetNW2String( "_schedule_" .. i .. "_5" ), math.floor( self:GetNW2Int( "_schedule_" .. i .. "_3" ) * 60 / 5 ) * 5 }
	end
...
-- Animation function
--------------------------------------------------------------------------------
function ENT:Animate( clientProp, value, min, max, speed, damping, stickyness, name )
	local id = clientProp
	...
	if anims[ id ].block then
		if anims[ id ].reload and IsValid( self.ClientEnts[ clientProp ] ) then
			self.ClientEnts[ clientProp ]:SetPoseParameter( name or "position", anims[ id ].value )
	...
	if IsValid( self.ClientEnts[ clientProp ] ) then self.ClientEnts[ clientProp ]:SetPoseParameter( name or "position", retval ) end
...
function ENT:AnimateFrom( clientProp, from, min, max )
	...
	if IsValid( self.ClientEnts[ clientProp ] ) then self.ClientEnts[ clientProp ]:SetPoseParameter( name or "position", val ) end
...
function ENT:ShowHide( clientProp, value, over )
	...
	if value == true and ( self.Hidden[ clientProp ] or over ) then
		...
		if not IsValid( self.ClientEnts[ clientProp ] ) and self:SpawnCSEnt( clientProp ) then self.UpdateRender = true end
...
hook.Add( "Think", "mplr-cabin-panel", function()
		...
		if GetConVar( "metrostroi_disablehovertext" ):GetInt() == 0 and ttdelay and ttdelay >= 0 then
			...
			if button then
				if ttdelay == 0 or CurTime() - lastAimButtonChange > ttdelay then
					...
					if convar == 0 and button.tooltipState and button.tooltip then toolTipText = toolTipText .. button.tooltipState( train ) end
				end
			end
		end
	end
...
net.Receive( "mplr-cabin-reset", function()
	local ent = net.ReadEntity()
	if IsValid( ent ) and ent.ClearButtons ~= nil then ent:ClearButtons() end
end )
...
local function handleCam( ply, button )
	...
	if button == KEY_LEFT then
		repeat
			...
			if train.CurrentCamera < 0 then train.CurrentCamera = #train.Cameras end
	...
	if button == KEY_RIGHT then
		repeat
			train.CurrentCamera = train.CurrentCamera + 1
			if train.CurrentCamera > #train.Cameras then train.CurrentCamera = 0 end
		until not train.Cameras[ train.CurrentCamera ] or not train.Cameras[ train.CurrentCamera ][ 4 ] or train:ShouldDrawPanel( train.Cameras[ train.CurrentCamera ][ 4 ] )
	end

	if button == KEY_DOWN then train.CurrentCamera = nil end
...
hook.Add( "PlayerButtonDown", "mplr-cabin-buttons", function( ply, key )
	handleKeyEvent( ply, key, true )
	handleCam( ply, key )
end )

hook.Add( "PlayerButtonUp", "mplr-cabin-buttons", function( ply, key ) handleKeyEvent( ply, key, false ) end )
if game.SinglePlayer() then
	net.Receive( "PlayerButtonDown_mplr", function()
		local key = net.ReadUInt( 16 )
		handleCam( LocalPlayer(), key )
		handleKeyEvent( LocalPlayer(), key, true )
	end )

	net.Receive( "PlayerButtonUp_mplr", function() handleKeyEvent( LocalPlayer(), net.ReadUInt( 16 ), false ) end )
end

local Gradient = Material( "vgui/gradient-d" )
local oldTrain
hook.Add( "HUDPaint", "mplr-draw-cameras", function()
	local train, outside = isValidTrainDriver( LocalPlayer() )
...
hook.Add( "HUDPaint", "mplr-draw-crosshair-tooltip", function()
