...
function ENT:CreateSignalModel()
	...
	self.Delta = self:LocalToWorld( self.Left and Vector( 0, -65, -8 ) or Vector( 0, 65, -8 ) )
	self.SignalModel:SetPos( self.Delta )
...
hook.Add( "PostDrawOpaqueRenderables", "MPLR_DrawSignals", function()
	...
	for _, ent in pairs( ents.GetAll() ) do
		if ent:GetClass() == "gmod_track_uf_signal" and IsValid( ent ) then ent:Draw() end
	end
end )
...
function ENT:SignalAspect( aspect )
	...
	elseif aspect == "Sh3d" then
		...
		--self:ClientSprites(pos_r, self.SpriteSize, Color(200, 0, 0,self.AlphaRed), true)
		--self:ClientSprites(pos_rr, self.SpriteSize, Color(200, 0, 0,alpha_rr), true)
		if RealTime() - blinkTime > 1 then
			lensOn = not lensOn
			blinkTime = RealTime()
		end

		self:ClientSprites( pos_r, self.SpriteSize, Color( 200, 0, 0 ), lensOn )
...
local debug = GetConVar( "metrostroi_drawsignaldebug" ):GetBool()
