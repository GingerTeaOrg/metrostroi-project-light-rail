...
function ENT:Initialize()
    ...
    -- Create couples
    self.FrontCouple = self:CreateCustomCoupler(Vector(465, 0, 12),
                                                Angle(0, 0, 0), true, "u2", "a")
...
function ENT:Think(dT)
    ...
    if IsValid(self.FrontBogey) and IsValid(self.MiddleBogey) and
        ...
        if self.Duewag_U3.ReverserLeverStateA == 3 or self:ReadTrainWire(6) < 1 or
            self.Duewag_U3.ReverserLeverStateA == -1 or
            self.Duewag_U3.ReverserLeverStateB == 3 or
            self.Duewag_U3.ReverserLeverStateB == -1 then
            ...
            if self.DepartureConfirmed == true then
                ...
                if self.Duewag_U3.ThrottleState < 0 then
                    ...
                    self.FrontBogey.BrakeCylinderPressure = self.Duewag_U3
                                                                .BrakePressure
                    self.MiddleBogey.BrakeCylinderPressure = self.Duewag_U3
                                                                 .BrakePressure
                    self.RearBogey.BrakeCylinderPressure = self.Duewag_U3
                                                               .BrakePressure
            ...
            if self:ReadTrainWire(8) < 1 then
                ...
                if self:ReadTrainWire(9) < 1 then
                    if self:ReadTrainWire(3) > 0 or self:ReadTrainWire(4) > 0 then
                        ...
                        if self:ReadTrainWire(4) < 1 and self:ReadTrainWire(3) >
                            0 then
                            self.FrontBogey.Reversed = false
                            self.RearBogey.Reversed = false
                            -- print(self:ReadTrainWire(4),self)
                        elseif self:ReadTrainWire(3) < 1 and
                            self:ReadTrainWire(4) > 0 then
                            self.FrontBogey.Reversed = true
                            self.RearBogey.Reversed = true
        ...
        elseif self.Duewag_U3.ReverserLeverStateA == -1 then
            if self.Duewag_U3.ThrottleState < 0 then
                ...
                self.RearBogey.BrakeCylinderPressure = self.Duewag_U3
                                                           .BrakePressure
            elseif self.Duewag_U3.ThrottleState > 0 and self.DepartureConfirmed ==
                true then
...
function ENT:OnButtonPress(button, ply)
    ...
    if self.Duewag_U3.ThrottleRateA == 0 then
        if button == "ThrottleUp" then self.Duewag_U3.ThrottleRateA = 3 end
        if button == "ThrottleDown" then
            self.Duewag_U3.ThrottleRateA = -3
        end
    end

    if self.Duewag_U3.ThrottleRateA == 0 then
        if button == "ThrottleUpFast" then
            self.Duewag_U3.ThrottleRateA = 10
        end
    ...
    if self.Duewag_U3.ThrottleRateA == 0 then
        if button == "ThrottleUpReallyFast" then
            self.Duewag_U3.ThrottleRateA = 20
        end
    ...
    if self.Duewag_U3.ThrottleRateA == 0 then
        if button == "ThrottleZero" then
            self.Duewag_U3.ThrottleStateA = 0
        end
    ...
    if button == "Throttle10Pct" then self.Duewag_U3.ThrottleStateA = 10 end

    if button == "Throttle20Pct" then self.Duewag_U3.ThrottleStateA = 20 end

    if button == "Throttle30Pct" then self.Duewag_U3.ThrottleStateA = 30 end
    if button == "Throttle40Pct" then self.Duewag_U3.ThrottleStateA = 40 end
    if button == "Throttle50Pct" then self.Duewag_U3.ThrottleStateA = 50 end
    if button == "Throttle60Pct" then self.Duewag_U3.ThrottleStateA = 60 end
    if button == "Throttle70Pct" then self.Duewag_U3.ThrottleStateA = 70 end
    if button == "Throttle80Pct" then self.Duewag_U3.ThrottleStateA = 80 end
    if button == "Throttle90Pct" then self.Duewag_U3.ThrottleStateA = 90 end

    if button == "Throttle10-Pct" then self.Duewag_U3.ThrottleStateA = -10 end

    if button == "Throttle20-Pct" then self.Duewag_U3.ThrottleStateA = -20 end

    if button == "Throttle30-Pct" then self.Duewag_U3.ThrottleStateA = -30 end
    if button == "Throttle40-Pct" then self.Duewag_U3.ThrottleStateA = -40 end
    if button == "Throttle50-Pct" then self.Duewag_U3.ThrottleStateA = -50 end
    if button == "Throttle60-Pct" then self.Duewag_U3.ThrottleStateA = -60 end
    if button == "Throttle70-Pct" then self.Duewag_U3.ThrottleStateA = -70 end
    if button == "Throttle80-Pct" then self.Duewag_U3.ThrottleStateA = -80 end
    if button == "Throttle90-Pct" then self.Duewag_U3.ThrottleStateA = -90 end
    ...
    if button == "ReverserUpSet" then
        if not self.Duewag_U3.ThrottleEngaged == true then
            if self.Duewag_U3.ReverserInsertedA == true then
                self.Duewag_U3.ReverserLeverStateA = self.Duewag_U3
                                                         .ReverserLeverStateA +
                                                         1
                self.Duewag_U3.ReverserLeverStateA =
                    math.Clamp(self.Duewag_U3.ReverserLeverStateA, -1, 3)
                -- self.Duewag_U3:TriggerInput("ReverserLeverState",self.ReverserLeverState)
                -- PrintMessage(HUD_PRINTTALK,self.Duewag_U3.ReverserLeverStateA)
            end
        end
    end
    if button == "ReverserDownSet" then
        if not self.Duewag_U3.ThrottleEngaged and
            self.Duewag_U3.ReverserInsertedA == true then
    ...
    if self.Duewag_U3.ReverserLeverStateB == 0 and
        self.Duewag_U3.ReverserLeverStateA == 0 then
        if button == "ReverserInsert" then
            if self.Duewag_U3.ReverserInsertedB and
                not self.Duewag_U3.ReverserInsertedA then
                self.Duewag_U3.ReverserInsertedA = true
                self.Duewag_U3.ReverserInsertedB = false
            elseif not self.Duewag_U3.ReverserInsertedB and
                self.Duewag_U3.ReverserInsertedA then
                self.Duewag_U3.ReverserInsertedA = false
            elseif not self.Duewag_U3.ReverserInsertedB and
                not self.Duewag_U3.ReverserInsertedA then
                self.Duewag_U3.ReverserInsertedA = true
            end
    ...
    if button == "BatteryToggle" then
        ...
        if self.BatteryOn == false and self.Duewag_U3.ReverserLeverStateA == 1 then
            self.BatteryOn = true
    ...
    if button == "BatteryDisableToggle" then
        if self.BatteryOn == true and self.Duewag_U3.ReverserLeverStateA == 1 then
            self.BatteryOn = false
    ...
    if button == "ThrowCouplerSet" then
        if self:ReadTrainWire(5) > 1 and self.Duewag_U3.Speed < 2 then
    ...
    if button == "HeadlightsToggle" then
        ...
        ----print(self.Duewag_U3.HeadlightsSwitch)
    end
    ...
    if button == "SetHoldingBrakeSet" then
        ...
        self.Duewag_U3.ManualRetainerBrake = true
    ...
    if button == "ReleaseHoldingBrakeSet" then
        self.Duewag_U3.ManualRetainerBrake = false
    end
...
function ENT:OnButtonRelease(button, ply)
    ...
    if button == "CycleIBISKey" then
        if self.Duewag_U3.IBISKeyA == false and self.Duewag_U3.IBISKeyATurned ==
            false then
            self.Duewag_U3.IBISKeyA = true
        elseif self.Duewag_U3IBISKeyA == true and self.Duewag_U3.IBISKeyATurned ==
            false then
            self.Duewag_U3.IBISKeyA = true
            self.Duewag_U3.IBISKeyATurned = true
        elseif self.Duewag_U3IBISKeyA == true and self.Duewag_U3.IBISKeyATurned ==
            true then
            self.Duewag_U3.IBISKeyA = true
            self.Duewag_U3.IBISKeyATurned = false
        end
    end

    if button == "RemoveIBISKey" then
        if self.Duewag_U3.IBISKeyA == true then
            self.Duewag_U3.IBISKeyA = false
        end
    end
    ...
    if (button == "ThrottleUp" and self.Duewag_U3.ThrottleRateA > 0) or
        (button == "ThrottleDown" and self.Duewag_U3.ThrottleRateA < 0) then
        self.Duewag_U3.ThrottleRateA = 0
    end
    if (button == "ThrottleUpFast" and self.Duewag_U3.ThrottleRateA > 0) or
        (button == "ThrottleDownFast" and self.Duewag_U3.ThrottleRateA < 0) then
        self.Duewag_U3.ThrottleRateA = 0
    end
