...
function ENT:Initialize()
	...
	-- Create couples
    self.FrontCouple = self:CreateCoupleUF(Vector( 475,0,15),Angle(0,0,0),true,"u2")	
    --self.RearCouple = self:CreateCoupleUF(Vector( 100,50,100),Angle(0,0,0),false,"dummy")	

	self.Async = true
	-- Create U2 Section B
	self.ptbsectionc = self:CreateSectionC(Vector(0,0,0))
...
function ENT:CreateSectionC(pos)
	local ang = Angle(0,0,0)
	local ptbsectionc = ents.Create("gmod_subway_uf_ptb_section_c")
	-- self.ptbsectionc = u2b
	ptbsectionc:SetPos(self:LocalToWorld(Vector(-210,0,0)))
	ptbsectionc:SetAngles(self:GetAngles() + ang)
	ptbsectionc:Spawn()
	ptbsectionc:SetOwner(self:GetOwner())
	local xmin = 5
	local xmax = 5
	...
	-- Add to cleanup list
	table.insert(self.TrainEntities,ptbsectionc)
	return ptbsectionc
end
