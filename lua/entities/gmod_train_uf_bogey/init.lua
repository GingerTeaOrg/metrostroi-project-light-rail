...
ENT.Types = {
    ["u5"] = {
        "models/lilly/uf/u5/bogey.mdl", Vector(0, 0.0, 0), Angle(0, 0, 0), nil,
        Vector(0, -61, -14), Vector(0, 61, -14), nil, Vector(4.3, -63, -3.3),
        Vector(4.3, 63, -3.3)
    },
    ["u2joint"] = {
        "models/lilly/uf/u2/jointbogey.mdl", Vector(0, 0.0, 0), Angle(0, 0, 0),
        nil, Vector(0, -61, -14), Vector(0, 61, -14), nil, Vector(4.3, -63, 0),
        Vector(4.3, 63, 0)
    },
    ["b_motor"] = {
    ...
    ["pt"] = {
        "models/lilly/uf/ptb/ptbogey.mdl", Vector(0, 0.0, 0), Angle(0, 0, 0),
        nil, Vector(0, -61, -14), Vector(0, 61, -14), nil,
        Vector(4.3, -63, -3.3), Vector(4.3, 63, -3.3)
    },
    ["def"] = {
...
function ENT:InitializeWheels()
    ...
    if CPPI then
        wheels:CPPISetOwner(self:CPPIGetOwner() or
                                self:GetNW2Entity("TrainEntity"):GetOwner())
    end
...
function ENT:GetDebugVars() return self.Variables end
...
function ENT:Couple(ent)
    if IsValid(constraint.AdvBallsocket(self, ent, 0, -- bone
    0, -- bone
    self.CouplingPointOffset, ent.CouplingPointOffset, 0, -- forcelimit
    0, -- torquelimit
    -25, -- xmin
    -10, -- ymin
    -25, -- zmin
    ...
    1 -- nocollide
        ...
        sound.Play("subway_trains/bogey/couple.mp3",
                   (self:GetPos() + ent:GetPos()) / 2, 70, 100, 1)
...
local function AreInCoupleDistance(ent1, ent2)
    return ent2:LocalToWorld(ent2.CouplingPointOffset):DistToSqr(
               ent1:LocalToWorld(ent1.CouplingPointOffset)) <
               COUPLE_MAX_DISTANCE
end
...
function ENT:StartTouch(ent)
    if CanCoupleTogether(self, ent) then self:Couple(ent) end
end
...
function ENT:ConnectDisconnect(status)
    ...
    if IsValid(train) then
        if status ~= nil then
            ...
            if (train.FrontCoupledBogeyDisconnect and isfront) or
                (train.RearCoupledBogeyDisconnect and not isfront) then
                train:OnBogeyConnect(self, isfront)
                if IsValid(self.CoupledBogey) then
                    self.CoupledBogey:ConnectDisconnect(true)
                end
                return
            end
            if (not train.FrontCoupledBogeyDisconnect and isfront) or
                (not train.RearCoupledBogeyDisconnect and not isfront) then
                train:OnBogeyDisconnect(self, isfront)
                if IsValid(self.CoupledBogey) then
                    self.CoupledBogey:ConnectDisconnect(false)
                end
                return
            end
        end
...
local function removeAdvBallSocketBetweenEnts(ent1, ent2)
    local constrainttable = constraint.FindConstraints(ent1, "AdvBallsocket")
    for k, v in pairs(constrainttable) do
        if (v.Ent1 == ent1 or v.Ent1 == ent2) and
            (v.Ent2 == ent1 or v.Ent2 == ent2) then v.Constraint:Remove() end
    end
end

function ENT:Decouple()
    if self.CoupledBogey then
        sound.Play("buttons/lever8.wav",
                   (self:GetPos() + self.CoupledBogey:GetPos()) / 2)
...
function ENT:OnCouple(ent)
    ...
    if IsValid(parent) then parent:OnCouple(ent, isforward) end
...
function ENT:OnDecouple()
    ...
    if IsValid(parent) then parent:OnDecouple(isforward) end
end

function ENT:Think()
    -- Re-initialize wheels
    if not IsValid(self.Wheels) or self.Wheels:GetNW2Entity("TrainBogey") ~=
        self then
        self:InitializeWheels()

        constraint.NoCollide(self.Wheels, self, 0, 0)
        if IsValid(self:GetNW2Entity("TrainEntity")) then
            constraint.NoCollide(self.Wheels, self:GetNW2Entity("TrainEntity"),
                                 0, 0)
        end
    end
    ...
    local localSpeed = -self:GetVelocity():Dot(self:GetAngles():Forward()) *
                           0.06858
    ...
    self.Acceleration = 0.277778 * (self.Speed - (self.PrevSpeed or 0)) /
                            self.DeltaTime
    ...
    local BrakeCP = (((self.BrakeCylinderPressure / 2.7 + pB / 1.6) ^
                        pneumaticPow) * 2.7) / 4.5 -- + (self.ParkingBrake and 1 or 0)
    if (BrakeCP * 4.5 > 1.5 - math.Clamp(math.abs(pitch) / 1, 0, 1)) and
        (absSpeed < 1) then
    ...
    local pneumaticFactor = math.Clamp(0.5 * self.Speed, 0, 1) *
                                (1 + math.Clamp((2 - self.Speed) / 2, 0, 1) *
                                    0.5)
    ...
    if BrakeCP >= 0.05 then
        ...
        pneumaticForce = -sign * pneumaticFactor * self.PneumaticBrakeForce *
                             BrakeCP * slopemulBr
    end
    ...
    local sideSpeed = -self:GetVelocity():Dot(self:GetAngles():Right()) *
                          0.06858
    ...
    if self.Reversed then
        self:GetPhysicsObject():ApplyForceCenter(
            self:GetAngles():Forward() * force + self:GetAngles():Right() *
                side_force)
    else
        self:GetPhysicsObject():ApplyForceCenter(
            -self:GetAngles():Forward() * force + self:GetAngles():Right() *
    ...
    if self.DisableSound < 2 then
        if self:GetNWBool("Async") then
            self:SetNW2Float("BrakeSqueal",
                             (self.BrakeCylinderPressure - 0.9) / 1.7)
        else
...
function ENT:SpawnFunction(ply, tr)
    ...
    if tr.Hit then
        ...
        if tracedata.Hit then
            -- Trackspawn
            pos = (tr.HitPos + tracedata.HitPos) / 2 +
...
function ENT:UpdateTextures()
    ...
    local level =
        math.random() > 0.95 and 0.7 or math.random() > 0.8 and 0.55 or
            math.random() > 0.35 and 0.25 or 0
    self:SetNW2Vector("DirtLevel",
                      math.Clamp(level + math.random() * 0.2 - 0.1, 0, 1))
end
