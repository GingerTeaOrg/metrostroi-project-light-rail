...
ENT.Types = {
	...
	[ "def" ] = { "models/lilly/mplr/bogeys/b-wagen/bogey_motor.mdl", Vector( 0, 0.0, 0 ), Angle( 0, 0, 0 ), "models/lilly/mplr/b_wheelset_traction.mdl", Vector( 0, -61, -28 ), Vector( 0, 61, -14 ), nil, Vector( 4.3, -63, -3.3 ), Vector( 4.3, 63, -3.3 ) }
}

function ENT:SetParameters()
	local typ = MPLR.BogeyTypes[ self.BogeyType ] or self.Types[ self.BogeyType ] or self.Types[ "def" ]
	if not typ then
		assert( "MPLR Bogey: Bogey type '" .. typ .. "' is not registered. Despawning." )
		self:Remove()
		return
	end
