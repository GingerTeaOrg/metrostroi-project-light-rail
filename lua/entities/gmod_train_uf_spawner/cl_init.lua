...
if not file.Exists("MPLR_train_spawner.txt","DATA") then
	file.Write("MPLR_train_spawner.txt","")
end
...
local function Draw()
	...
	CreateSlider("WagNum",0,1, GetGlobalInt("mplr_maxwagons"),Metrostroi.GetPhrase("Spawner.WagNum"),function(slider)
		local WagNumTable
		for k,name in pairs(UF.TrainClasses) do
			local ENT = scripted_ents.Get(name)
			if not ENT.Spawner or ENT.ClassName ~= Settings.Train  then continue end
...
net.Receive("MPLRMaxWagons", function()
	...
	if trainTypeT and trainTypeT:IsValid() then
		trainTypeT:SetText(Format("%s(%d/%d)\n%s:%d",Metrostroi.GetPhrase("Spawner.Trains1"),GetGlobalInt("mplr_train_count"),MaxWagons,Metrostroi.GetPhrase("Spawner.Trains2"),MaxWagonsOnPlayer))
	end
end)
net.Receive("MPLRTrainCount", function()
	if trainTypeT and trainTypeT:IsValid() then
		trainTypeT:SetText(Format("%s(%d/%d)\n%s:%d",Metrostroi.GetPhrase("Spawner.Trains1"),GetGlobalInt("mplr_train_count"),MaxWagons,Metrostroi.GetPhrase("Spawner.Trains2"),MaxWagonsOnPlayer))
	end
end)
