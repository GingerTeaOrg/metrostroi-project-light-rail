...
if not file.Exists( "MPLR_train_spawner.txt", "DATA" ) then file.Write( "MPLR_train_spawner.txt", "" ) end
local function UpdateConCMD()
	file.Write( "MPLR_train_spawner.txt", util.TableToJSON( Settings, true ) )
end
...
local function CreateList( name, text, tbl, OnSelect, stbl )
	...
	List.Call = OnSelect
	List:SetPos( 175 + 270 * math.floor( Pos / MaxHorisontal ), 28 + 24 * ( Pos % MaxHorisontal ) )
	...
	if not List:GetOptionData( 1 ) then
		ListLabel:Remove()
		List:Remove()
		return
	end
...
local function CreateSlider( name, decimals, min, max, text, OnSelect, stbl )
	...
	Slider:SetPos( 25 + 270 * math.floor( Pos / MaxHorisontal ), 28 + 24 * ( Pos % MaxHorisontal ) - 7 + 4 )
	Slider:SetWide( 290 )
	Slider:SetTall( 28 )
...
local function UpdateTrainList( fromPresets )
	...
	for k, name in pairs( UF.TrainClasses ) do
		...
		for i, menu in ipairs( ENT.Spawner ) do
			if menu[ 3 ] == "List" then
				if Settings[ Settings.Train ][ menu[ 1 ] ] == nil then Settings[ Settings.Train ][ menu[ 1 ] ] = menu[ 5 ] end
				CreateList( menu[ 1 ], menu[ 2 ], menu[ 4 ], menu[ 7 ] )
			elseif menu[ 3 ] == "Boolean" then
				if Settings[ Settings.Train ][ menu[ 1 ] ] == nil then Settings[ Settings.Train ][ menu[ 1 ] ] = menu[ 4 ] end
				CreateCheckBox( menu[ 1 ], menu[ 2 ], menu[ 6 ] )
			elseif menu[ 3 ] == "Slider" then
				if Settings[ Settings.Train ][ menu[ 1 ] ] == nil then Settings[ Settings.Train ][ menu[ 1 ] ] = menu[ 7 ] end
...
local function Draw()
	...
	for _, name in pairs( UF.TrainClasses ) do
		...
		Trains[ ENT.ClassName ] = ENT.SubwayTrain.Manufacturer .. " " .. ENT.SubwayTrain.Name
	end
	...
	--[[CreateSlider( "WagNum", 0, 1, GetGlobalInt( "mplr_maxwagons" ), "Car Count									",
		...
		if WagNumTable then
			...
			for i = 2, #WagNumTable do
				if WagNumTable[ i ] <= math.Round( slider:GetValue(), 0 ) then retval = WagNumTable[ i ] end
			end
			return retval
		end
	end, Settings )]]
...
local function getPresetName( name, presets, ignoreOwn )
	...
	for i, v in ipairs( presets ) do
		...
		if ( not ignoreOwn or i ~= presets.Selected ) and ( v.PresetName == name or name2 == name ) then maxrep = math.max( 1, tonumber( nameid ) or 1 ) end
	end
...
local function createPresetsFrame()
	...
	pFrame:SetSize( 300, 58 + 24 * 1 )
	--pFrame:Center()
	pFrame.OnRemove = function() if IsValid( WFrame ) then VGUI.WFrame() end end
	--[[local ListLabel = vgui.Create("DLabel", frame)
	--	ListLabel:SetPos(5 + 300*math.floor(Pos/MaxHorisontal), 24+24*(Pos%MaxHorisontal))
	ListLabel:SetPos(5 + 270*math.floor(Pos/MaxHorisontal),24+24*(Pos%MaxHorisontal))
...
local function createWagonsFrame()
	...
	wFrame.OnRemove = function() if IsValid( PFrame ) then VGUI.PFrame() end end
	VGUI.WFrame()
end

local function createFrame()
	MaxWagons = GetGlobalInt( "mplr_maxtrains" ) * GetGlobalInt( "mplr_maxwagons" )
	MaxWagonsOnPlayer = GetGlobalInt( "mplr_maxtrains_onplayer" ) * GetGlobalInt( "mplr_maxwagons" )
	...
	frame:SetTitle( "Project Light Rail Spawner" )
	...
	VGUI[ "Close" ] = function() if IsValid( Close ) and IsValid( frame ) then Close:SetPos( 5, frame:GetTall() - Close:GetTall() - 5 ) end end
	...
	VGUI[ "spawn" ] = function() if IsValid( spawn ) and IsValid( frame ) then spawn:SetPos( frame:GetWide() - Close:GetWide() - 5, frame:GetTall() - Close:GetTall() - 5 ) end end
	spawn.DoClick = function()
		...
		net.WriteTable( tbl )
		net.SendToServer()
		local tool = LocalPlayer():GetTool( "gm_ufspawner" )
		tool.Settings = tbl
...
net.Receive( "train_spawner_uf_open", function()
...
net.Receive( "MPLRTrainSpawner", createFrame )
net.Receive( "MPLRMaxWagons", function()
	MaxWagons = GetGlobalInt( "mplr_maxtrains" ) * GetGlobalInt( "mplr_maxwagons" )
	MaxWagonsOnPlayer = GetGlobalInt( "mplr_maxtrains_onplayer" ) * GetGlobalInt( "mplr_maxwagons" )
	if trainTypeT and trainTypeT:IsValid() then trainTypeT:SetText( Format( "%s(%d/%d)\n%s:%d", Metrostroi.GetPhrase( "Spawner.Trains1" ), GetGlobalInt( "mplr_train_count" ), MaxWagons, Metrostroi.GetPhrase( "Spawner.Trains2" ), MaxWagonsOnPlayer ) ) end
end )

net.Receive( "MPLRTrainCount", function() if trainTypeT and trainTypeT:IsValid() then trainTypeT:SetText( Format( "%s(%d/%d)\n%s:%d", Metrostroi.GetPhrase( "Spawner.Trains1" ), GetGlobalInt( "mplr_train_count" ), MaxWagons, Metrostroi.GetPhrase( "Spawner.Trains2" ), MaxWagonsOnPlayer ) ) end end )