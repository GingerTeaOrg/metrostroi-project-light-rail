...
function ENT:DrawFullLense()
	...
	for j = 1, seg do
		surface.DrawPoly( {
			...
			{
				x = x + math.sin( math.rad( ( j - 1 ) * ang ) ) * r,
				y = y - math.cos( math.rad( ( j - 1 ) * ang ) ) * r
			},
			{
				x = x + math.sin( math.rad( j * ang ) ) * r,
				y = y - math.cos( math.rad( j * ang ) ) * r
			},
		} )
	end
end
...
local function CallDrawRender( self, translucent )
	...
	for k, v in ipairs( data.Lenses ) do
		...
		if Render then Render( self, LocalToWorld( v[ 1 ], v[ 2 ], pos, ang ) ) end
...
function ENT:DrawSprites()
	...
	for k, v in ipairs( data.Lenses ) do
		local lense, ldata = self:GetLenseData( k )
		if ldata.DrawSprites then ldata.DrawSprites( self, LocalToWorld( v[ 1 ], v[ 2 ], pos, ang ) ) end
	end
end
...
function ENT:UpdateClientEnts( transmit )
	if transmit == nil then transmit = not self:IsDormant() end
	...
	if transmit then self:UpdateLenseModels() end
end
...
function ENT:UpdateLenseModels()
	...
	for i = 1, #lenses do
		...
		local lense, ldata = self:GetLenseData( k )
		if not ldata.ClientEnts then continue end
		local clents = ldata.ClientEnts
		for j = 1, #clents do
			local k2, v2 = j, clents[ j ]
			if not v2.ShouldDraw or v2.ShouldDraw( self ) then
				...
				if not IsValid( ent ) then
					...
					if v2.Initialize then v2.Initialize( self, ent ) end
				end

				if v2.Update then v2.Update( self, ent ) end
...
function ENT:OnLenseUpdate( num, old, new )
	if self.RenderClientEnts then self:ClearLenseModels( num ) end
end