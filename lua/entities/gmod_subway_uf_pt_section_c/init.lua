...
-- 2 = Only intherim                                
---------------------------------------------------
ENT.SubwayTrain = {Type = "P8", Name = "Pt", WagType = 2, Manufacturer = "Duewag"}
...
function ENT:OnButtonPress(button, ply) end
...
function ENT:CreateSectionA(pos)
	...
	constraint.Axis(self.FrontBogey, SecA, 0, -- bone
	0, -- bone
	Vector(0, 0, 0), Vector(8, 0, 14), 0, -- forcelimit
	0, -- torquelimit
	0, 1 -- nocollide
	)
	-- Add to cleanup list
...
function ENT:CreateSectionB(pos)
	...
	constraint.Axis(self.RearBogey, SecB, 0, -- bone
	0, -- bone
	Vector(0, 0, 0), Vector(-8, 0, 14), 0, -- forcelimit
	0, -- torquelimit
	0, 1 -- nocollide
...
function ENT:CreateBogeyUF_a(pos, ang, forward, typ)
	...
	if self.NoPhysics then
		...
		constraint.Axis(bogey, self.SectionA, 0, 0, Vector(0, 0, 0), Vector(0, 0, 0), 0, 0, 0, 1, Vector(0, 0, 1), false)
		if forward and IsValid(self.FrontCouple) then
			constraint.NoCollide(bogey, self.FrontCouple, 0, 0)
...
function ENT:CreateCoupleUF_a(pos, ang, forward, typ)
	...
	if self.NoPhysics then
		...
		constraint.AdvBallsocket(self.SectionA, coupler, 0, -- bone
		0, -- bone
		...
		if forward and IsValid(self.FrontBogey) then constraint.NoCollide(self.FrontBogey, coupler, 0, 0) end
		--[[
