...
function ENT:GetPairedControllers()
	if not type(self.VMF.PairedControllerLeft) == "string" or type(self.VMF.PairedControllerRight) == "string" then return end
...
function ENT:Think()
	...
	if not self.SwitchID then return end -- if we don't know the SwitchID then what's the point
	...
	local found, dump2, TrainDetected = UF.IsTrackOccupied(self.TrackPos.node1, self.TrackPos.x, self.TrackPos.x < TargetX, "light", TargetX) -- we only need the first and second returned values, so only take that
	...
	if route and line and not override then
		...
		if routeCommand == "left" and leftController then
			self:TriggerPairedController(self.PairedControllers[1][2], self.PairedControllers[1][1])
		elseif routeCommand == "right" and rightController then
	...
	self:NextThink(CurTime() + 1.0) -- make sure this runs reliably every second.
end
