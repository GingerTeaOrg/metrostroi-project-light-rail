...
function ENT:OnRemove()
	...
	self.PlatformStart = self:GetNW2Vector( "PlatformStart", Vector( 0, 0, 0 ) )
	self.PlatformEnd = self:GetNW2Vector( "PlatformEnt", Vector( 0, 0, 0 ) )
end
...
function ENT:Think()
	...
	if self.PlatformStart == Vector( 0, 0, 0 ) then self.PlatformStart = self:GetNW2Vector( "PlatformStart", false ) end
	if self.PlatformEnd == Vector( 0, 0, 0 ) then self.PlatformEnd = self:GetNW2Vector( "PlatformEnd", false ) end
	-- Platform parameters
...
PassStaySequences = { "idle_all_01", "idle_all_02", "pose_standing_01", "pose_standing_02", }
function ENT:CreateNewPassenger()
	local ent = ClientsideModel( table.Random( passengerModels ), RENDERGROUP_BOTH )
	ent:SetSequence( ent:LookupSequence( table.Random( PassStaySequences ) ) or 0 )
	ent.RenderOverride = function( self ) if EyePos():DistToSqr( self:GetPos() ) < 1024 ^ 2 then self:DrawModel() end end
