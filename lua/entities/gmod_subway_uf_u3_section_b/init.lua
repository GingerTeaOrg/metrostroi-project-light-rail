...
function ENT:Initialize()
	...
	[66] = { "light",Vector(-480.32,-30.9657,41.43), Angle(0,0,0), Color(255,102,0),     brightness = 0.9, scale = 0.1, texture = "sprites/light_glow02.vmt" }, --brake light left
	[67] = { "light",Vector(-480.32,30.9657,41.43), Angle(0,0,0), Color(255,102,0),     brightness = 0.9, scale = 0.1, texture = "sprites/light_glow02.vmt" }, --brake light right
...
function ENT:OnButtonPress(button,ply)
	...
	if button == "Throttle10Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 10
	end
	
	if button == "Throttle20Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 20
	end
	
	if button == "Throttle30Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 30
	end
	if button == "Throttle40Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 40
	end
	if button == "Throttle50Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 50
	end
	if button == "Throttle60Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 60
	end
	if button == "Throttle70Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 70
	end
	if button == "Throttle80Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 80
	end
	if button == "Throttle90Pct" then
		self.ParentTrain.Duewag_U3.ThrottleState = 90
	end
	...
	if button == "PantographRaiseSet" then
		...
		if self.ParentTrain.Duewag_U3.BatteryOn == true then
			self.PantoUp = true
		end
	...
	if button == "ReverserUpSet" then
		if 
		not self.ParentTrain.Duewag_U3.ThrottleEngaged then
	...
	if button == "ReverserDownSet" then
		if 
		not self.ParentTrain.Duewag_U3.ThrottleEngaged and self.ParentTrain.Duewag_U3.ReverserInsertedB == true then
	...
	if button == "ThrowCouplerSet" then
		if self:ReadTrainWire(5) > 1 and self.ParentTrain.Duewag_U3.Speed < 2 then
	...
	if button == "ReleaseHoldingBrakeSet" then
		...
		self.ParentTrain.Duewag_U3.ManualRetainerBrake = false
	end
...
function ENT:OnButtonRelease(button,ply)
	...
	if button == "CycleIBISKey" then
		if self.ParentTrain.Duewag_U3.IBISKeyA == false and self.ParentTrain.Duewag_U3.IBISKeyATurned == false then
			self.ParentTrain.Duewag_U3.IBISKeyA = true
		elseif self.ParentTrain.Duewag_U3IBISKeyA == true and self.ParentTrain.Duewag_U3.IBISKeyATurned == false then
			self.ParentTrain.Duewag_U3.IBISKeyA = true
			self.ParentTrain.Duewag_U3.IBISKeyATurned = true
		elseif self.ParentTrain.Duewag_U3IBISKeyA == true and self.ParentTrain.Duewag_U3.IBISKeyATurned == true then
			self.ParentTrain.Duewag_U3.IBISKeyA = true
			self.ParentTrain.Duewag_U3.IBISKeyATurned = false
		end
	end

	if button == "RemoveIBISKey" then
		if self.ParentTrain.Duewag_U3.IBISKeyA == true then
			self.ParentTrain.Duewag_U3.IBISKeyA = false
		end
	end
	...
	if (button == "ThrottleUp" and self.ParentTrain.Duewag_U3.ThrottleRate > 0) or (button == "ThrottleDown" and self.ParentTrain.Duewag_U3.ThrottleRate < 0) then
		self.ParentTrain.Duewag_U3.ThrottleRate = 0
	end
	if (button == "ThrottleUpFast" and self.ParentTrain.Duewag_U3.ThrottleRate > 0) or (button == "ThrottleDownFast" and self.ParentTrain.Duewag_U3.ThrottleRate < 0) then
		self.ParentTrain.Duewag_U3.ThrottleRate = 0
	end
