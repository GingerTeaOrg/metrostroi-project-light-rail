...
function ENT:Think()
    ...
    if not IsValid( self.ParentTrain ) then -- we don't do anything without the A section
        return
    end
...
function ENT:Animations()
    ...
    self:ShowHide( "reverser", self.ParentTrain:GetNW2Bool( "ReverserInsertedB", false ) )
