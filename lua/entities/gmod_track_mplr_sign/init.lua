...
function ENT:Initialize()
	...
	if not self:GetNW2String("Model") then
		self:SetNW2String("Model",self.BasePath .. "/" .. self.Type .. ".mdl")
	end
end

function ENT:FindNextSign()
	if not self.TrackPosition then return end
	return UF.ScanTrackForEntity("gmod_track_mplr_sign", self.Node, self.TrackPosition.x, self.Forward, nil, true, "Type", "speed", true)
end
