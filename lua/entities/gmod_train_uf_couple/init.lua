...
ENT.Types = {
    ...
    ["b"] = {
        "models/lilly/mplr/couplers/b-wagen_coupler.mdl", Vector(-74.1, 0, 0)
    },
    ["def"] = {
        "models/lilly/mplr/couplers/b-wagen_coupler.mdl", Vector(-74.1, 0, 0)
    }
...
function ENT:SetParameters()
    local typ = self.Types[self.CoupleType or "def"] -- TODO: implement dynamic coupler model injection

    self:SetModel(typ and typ[1] or
                      "models/lilly/mplr/couplers/b-wagen_coupler.mdl")
    self.CouplingPointOffset = typ and typ[2] or Vector(-370, 0, 0)
end
...
function ENT:OnRemove() if self.CoupledEnt ~= nil then self:Decouple() end end
    ...
    for k, v in pairs(constrainttable) do
        if v.Type == "Weld" then
            if ((v.Ent1 == ent1 or v.Ent1 == ent2) and
                (v.Ent2 == ent1 or v.Ent2 == ent2)) then
                coupled = true
            end
        end
    end
...
function ENT:Couple(ent)
    ...
    if IsValid(constraint.Weld(self, ent, 0, -- bone
    0, -- bone
    ...
    -- 1 --nocollide
        ...
        sound.Play("lilly/uf/common/coupling.mp3",
...
local function AreInCoupleDistance(ent, self)
    print(self:LocalToWorld(self.CouplingPointOffset):DistToSqr(
              ent:LocalToWorld(ent.CouplingPointOffset)))
    return self:LocalToWorld(self.CouplingPointOffset):DistToSqr(
               ent:LocalToWorld(ent.CouplingPointOffset)) < COUPLE_MAX_DISTANCE
end
...
function ENT:ConnectDisconnect(status)
    ...
    if IsValid(train) then
        if status ~= nil then
            ...
            if (train.FrontCoupledCouplerDisconnect and isfront) or
                (train.RearCoupledCouplerDisconnect and not isfront) then
                train:OnCouplerConnect(self, isfront)
                if IsValid(self.Coupled) then
                    self.CoupledEnt:ConnectDisconnect(true)
                end
                return
            end
            if (not train.FrontCoupledCouplerDisconnect and isfront) or
                (not train.RearCoupledCouplerDisconnect and not isfront) then
                train:OnCouplerDisconnect(self, isfront)
                if IsValid(self.Coupled) then
                    self.CoupledEnt:ConnectDisconnect(false)
                end
                return
            end
        end
...
function ENT:Decouple()
    if IsValid(self.CoupledEnt) then
        sound.Play("buttons/lever8.mp3",
                   (self:GetPos() + self.CoupledEnt:GetPos()) / 2)
