...
ENT.Types = {
	...
	[ "b" ] = { "models/lilly/mplr/couplers/b-wagen_coupler.mdl", Vector( 74.1, 0, 0 ) },
	[ "def" ] = { "models/lilly/mplr/couplers/b-wagen_coupler.mdl", Vector( -74.1, 0, 0 ) }
}

function ENT:SetParameters()
	local typ = self.Types[ self.CoupleType or "def" ] -- TODO: implement dynamic coupler model injection
	self:SetModel( typ and typ[ 1 ] or "models/lilly/mplr/couplers/b-wagen_coupler.mdl" )
	self.CouplingPointOffset = typ and typ[ 2 ] or Vector( 70, 0, 0 )
end
...
function ENT:OnRemove()
	if self.CoupledEnt ~= nil then self:Decouple() end
end
...
local function AreCoupled( ent1, ent2 )
	...
	for k, v in pairs( constrainttable ) do
		if v.Type == "Weld" then if ( v.Ent1 == ent1 or v.Ent1 == ent2 ) and ( v.Ent2 == ent1 or v.Ent2 == ent2 ) then coupled = true end end
	end
	return coupled
end
...
function ENT:Couple( ent )
	...
	if IsValid( constraint.Weld( self, ent, 0, 0, 0 ) ) then -- bone -- bone -- self.CouplingPointOffset, -- ent.CouplingPointOffset, -- forcelimit ----0, --torquelimit ---25, --xmin ---10, --ymin ---25, --zmin -- 25, --xmax -- 10, --ymax -- 25, --zmax -- 0, --xfric -- 0, --yfric -- 0, --zfric -- 0, --rotonly -- 1 --nocollide
...
local function CanCoupleTogether( ent1, ent2 )
	...
	if ent2:GetModel() ~= ent1:GetModel() then -- only couple if the two couplers are of the same type
		return false
	end
...
function ENT:GetConnectDisconnect()
	...
	local train = self:GetNW2Entity( "TrainEntity" )
	if IsValid( train ) then
		if ( train.FrontCoupledCouplerDisconnect and isfront ) or ( train.RearCoupledCouplerDisconnect and not isfront ) then return false end
		if ( not train.FrontCoupledCouplerDisconnect and isfront ) or ( not train.RearCoupledCouplerDisconnect and not isfront ) then return true end
	end
end
...
local function removeAdvBallSocketBetweenEnts( ent1, ent2 )
	local constrainttable = constraint.FindConstraints( ent1, "Weld" )
	for k, v in pairs( constrainttable ) do
		if ( v.Ent1 == ent1 or v.Ent1 == ent2 ) and ( v.Ent2 == ent1 or v.Ent2 == ent2 ) then v.Constraint:Remove() end
	end
end
