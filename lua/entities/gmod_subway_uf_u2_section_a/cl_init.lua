...
ENT.ButtonMap["Left"] = {
    ...
    z=-3, ang=0,
    var="ReduceBrake",speed=1, vmin=0, vmax=1,
    sndvol = 1, snd = function(val) return val and "button_on" or "button_off" end,sndmin = 80, sndmax = 1e3/3, sndang = Angle(-90,0,0),},
    },
    }
}
...
function ENT:Think()
    ...
    elseif self:GetNW2String("Texture","") == "OrEbSW" then
        ...
        if IsValid(leftNum1) and IsValid(middleNum2) and IsValid(rightNum3) then
        ...
        if IsValid(decal2) then
            decal2:SetPos(self:LocalToWorld(Vector(0,0,-8)))
        end
        if IsValid(decal) then
            decal:SetPos(self:LocalToWorld(Vector(0,0,-8)))
        end
    else
