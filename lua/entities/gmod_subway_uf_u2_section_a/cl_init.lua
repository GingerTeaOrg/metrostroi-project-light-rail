	...
	buttons = {
		{
			...
			model = {var = "OpenBOStrab", sndvol = 1, sndmin = 90, sndmax = 1e3, sndang = Angle(-90, 0, 0)}
		}
	}
}
...
function ENT:Think()
	...
	elseif self:GetNW2String("Texture", "") == "OrEbSW" or self:GetNW2String("Texture", "") == "OrEbSW_2" then
		...
		local leftNum, middleNum, rightNum = self.ClientEnts["carnumber1"], self.ClientEnts["carnumber2"], self.ClientEnts["carnumber3"]
		if IsValid(self.SectionB) then
			local leftNum1, middleNum2, rightNum3 = self.SectionB.ClientEnts["carnumber1"], self.SectionB.ClientEnts["carnumber2"], self.SectionB.ClientEnts["carnumber3"]
		end
