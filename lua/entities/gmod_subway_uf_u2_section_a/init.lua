...
function ENT:CreateBogeyUF(pos, ang, forward, typ)
    ...
    if self.NoPhysics then
        ...
        constraint.Axis(bogey, self, 0, 0, Vector(0, 0, 0), Vector(0, 0, 0), 0,
                        0, 0, 1, Vector(0, 0, 1), false)
    end
    -- Add to cleanup list
...
function ENT:CreateCoupleUF(pos, ang, forward, typ)
    ...
    constraint.Axis(coupler, self, 0, 0, Vector(0, 0, 0), Vector(0, 0, 0), 0, 0,
                    0, 1, Vector(0, 0, 1), false)
...
function ENT:Initialize()
    ...
    -- Create couples
    self.FrontCouple = self:CreateCoupleUF(Vector(415, 0, 0), Angle(0, 0, 0),
                                           true, "u2")
    ...
    self.RearBogey = self:CreateBogeyUF_b(Vector(-290, 0, 0), Angle(0, 180, 0),
                                          false, "duewag_motor")

    self.RearCouple = self:CreateCouplerUF_b(Vector(-415, 0, 0),
...
function ENT:Think(dT)
    ...
    if self:ReadTrainWire(7) > 0 or self:ReadTrainWire(6) > 0 or
        self.Duewag_U2.BatteryOn == true then -- if the battery is on

        if self.DoorSideUnlocked == "Left" and self.DoorsUnlocked == true and
            self.Door1 ~= true then
        ...
        elseif self.DoorSideUnlocked == "Right" and self.DoorsUnlocked == true and
            self.Door1 ~= true then
            if self.Duewag_U2.VZ then
        ...
        elseif self.DoorSideUnlocked == "Left" and self.Door1 == true then
            ...
            self:SetNW2Bool("DoorsClosing", false)
        elseif self.DoorSideUnlocked == "Left" and self.DoorsUnlocked == false and
            self.Door1 ~= true then
            if self.Duewag_U2.VZ then
                self:WriteTrainWire(13, 1)
                self:WriteTrainWire(14, 0)
        ...
        elseif self.DoorSideUnlocked == "Right" and self.DoorsUnlocked == false and
            self.Door1 ~= true then
        ...
        if self:ReadTrainWire(6) > 0 then
            if self:ReadTrainWire(13) > 0 and self:ReadTrainWire(14) < 1 and
                self:ReadTrainWire(15) > 0 then
                self:DoorHandler(true, true, false, false)
                self:SetNW2Bool("DoorsClosing", false)
            elseif self:ReadTrainWire(14) > 0 and self:ReadTrainWire(13) < 1 and
                self:ReadTrainWire(15) > 0 then
                self:DoorHandler(true, false, true, false)
                self:SetNW2Bool("DoorsClosing", false)
            elseif self:ReadTrainWire(13) > 0 and self:ReadTrainWire(14) < 1 and
                self:ReadTrainWire(15) < 1 then
                self:DoorHandler(false, true, false, false)
                self:SetNW2Bool("DoorsClosing", true)
        ...
        if self:GetNW2Bool("Braking", true) == true and
            self:GetNW2Bool("AIsCoupled", false) == false and
            self:ReadTrainWire(3) < 1 and self:ReadTrainWire(20) < 1 and
            self:ReadTrainWire(21) < 1 then
            self:SetLightPower(56, true)
            self:SetLightPower(57, true)
            self:SetNW2Bool("BrakeLights", true)

        elseif self:GetNW2Bool("AIsCoupled", false) == true and
        ...
        if self.Panel.Headlights > 0 or self:ReadTrainWire(7) > 0 and
            self:ReadTrainWire(33) > 0 then
            ...
            elseif self.Duewag_U2.VE == true then
                self:WriteTrainWire(31,
                                    self.Duewag_U2.ReverserState > 0 and 1 or 0)
                self:WriteTrainWire(32,
                                    self.Duewag_U2.ReverserState < 0 and 1 or 0)
            end
        ...
        elseif (self.DoorsUnlocked == true) or self:ReadTrainWire(15) > 0 then
            ...
            if (self.DoorSideUnlocked == "Left") or self:ReadTrainWire(13) > 0 and
                self:ReadTrainWire(15) > 0 then
                ...
                self.SectionB:SetLightPower(36, false)
                self.SectionB:SetLightPower(37, false)
            elseif (self.DoorSideUnlocked == "None") or self:ReadTrainWire(15) <
                1 and self:ReadTrainWire(13) < 1 and self:ReadTrainWire(14) < 1 then
                self:SetLightPower(30, false)
                self:SetLightPower(31, false)
                self:SetLightPower(32, false)
                self:SetLightPower(33, false)
                ...
                self.SectionB:SetLightPower(36, false)
                self.SectionB:SetLightPower(37, false)
            elseif (self.DoorSideUnlocked == "Right") or self:ReadTrainWire(14) >
                0 and self:ReadTrainWire(15) > 0 then
