...
function ENT:Initialize()
	...
	-- Create bogeys
	self.FrontBogey = self:CreateBogeyUF(Vector( 425,0.8,0),Angle(0,180,0),true,"u5")
    self.RearBogey  = self:CreateBogeyUF(Vector(1.6,0,0),Angle(0,0,0),false,"u5joint")
    self.FrontBogey:SetNWBool("Async",true)
    self.RearBogey:SetNWBool("Async",true)
	-- Create couples
    self.FrontCouple = self:CreateCoupleUF(Vector( 500,0,15),Angle(0,0,0),true,"u2")	
    self.RearCouple = self:CreateCoupleUF(Vector( 100,0,100),Angle(0,0,0),true,"u2")	
	self.Async = true
	-- Create U2 Section B
	self.u5sectionb = self:CreateSectionB(Vector(-660,0,-140))
...
function ENT:CreateSectionB(pos)
	...
	local u5sectionb = ents.Create("gmod_subway_uf_u5_50_section_b")
	-- self.u5sectionb = u2b
	u5sectionb:SetPos(self:LocalToWorld(Vector(1.7,0,-0.5)))
	u5sectionb:SetAngles(self:GetAngles() + ang)
	u5sectionb:Spawn()
	u5sectionb:SetOwner(self:GetOwner())
	local xmin = 5
	local xmax = 5
	--if button == "RearDoor" then u5sectionb.RearDoor = not u5sectionb.RearDoor end	
	...
	-- Add to cleanup list
	table.insert(self.TrainEntities,u5sectionb)
	return u5sectionb
end
