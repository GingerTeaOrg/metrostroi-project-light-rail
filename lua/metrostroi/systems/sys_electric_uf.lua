...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem( "Electric_MPLR" )
function TRAIN_SYSTEM:Initialize()
	...
	self.EnergyChange = 0
	-- Signal resistor
	self.RPSignalResistor = FailSim.AddParameter( self, "SignalResistor", {
		value = 1.0,
		precision = 0.20
	} )
...
function TRAIN_SYSTEM:Think( dT, iter )
	...
	-- Solve circuits
	----------------------------------------------------------------------------
	if self.ThyristorController then self:SolveThyristorController( Train, dT ) end
...
function TRAIN_SYSTEM:SolveThyristorController( Train, dT )
	...
	elseif not Active then
		...
		local T = 180.0 + ( 100.0 * Train.Pneumatic.WeightLoadRatio + 80.0 ) * Train:ReadTrainWire( 
...
function TRAIN_SYSTEM:SolvePowerCircuits( Train, dT, firstIter )
	...
	if Train.PositionSwitch.SelectedPosition == 1 then -- PS
		...
		self.R3 = 0.0
	elseif Train.PositionSwitch.SelectedPosition == 2 then
		-- PP
		self.R1 = Train.ResistorBlocks.R1C2( Train )
		self.R2 = Train.ResistorBlocks.R2C2( Train )
		self.R3 = 0.0
	elseif Train.PositionSwitch.SelectedPosition >= 3 then
		-- PT
		self.R1 = Train.ResistorBlocks.R1C1( Train )
		self.R2 = Train.ResistorBlocks.R2C1( Train )
...
function TRAIN_SYSTEM:SolvePS( Train )
	...
	local Rtotal = self.Ranchor13 + self.Ranchor24 + self.Rstator13 + self.Rstator24 + self.R1 + self.R2 + self.R3 + self.ExtraResistanceLK5
