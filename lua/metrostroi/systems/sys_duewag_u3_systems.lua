...
function TRAIN_SYSTEM:Think(Train)
		...
		if IsValid(self.Train.SectionB) then self.Train.SectionB:SetNW2Float("ThrottleAnim", self.ThrottleStateAnimB) end
		...
		if self.ReverserLeverStateB == 1 then
				self.Train.SectionB:SetNW2Float("ReverserAnimate", 0.4)

		elseif self.ReverserLeverStateB == 0 then
				self.Train.SectionB:SetNW2Float("ReverserAnimate", 0.25)

		elseif self.ReverserLeverStateB == 3 then
				self.Train.SectionB:SetNW2Float("ReverserAnimate", 1)

		elseif self.ReverserLeverStateB == 2 then
				self.Train.SectionB:SetNW2Float("ReverserAnimate", 0.7)

		elseif self.ReverserLeverStateB == -1 then
				self.Train.SectionB:SetNW2Float("ReverserAnimate", 0)
		end
		...
		if self.EmergencyBrake == true then
				if self.TractionConditionFulfilled == true then self.Train:WriteTrainWire(10, 1) end
		else
		...
		if self.Train:GetNW2Bool("BatteryOn", false) == true or self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(7) > 0 then -- if either the battery is on or the EMU cables signal multiple unit mode
				...
				if self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(12) > 0 and self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) == false then
						self.TractionConditionFulfilled = true
				elseif self.Train:ReadTrainWire(6) < 1 and self.Train.DeadmanUF.IsPressed > 0 then
						self.TractionConditionFulfilled = true
				elseif self.Train:ReadTrainWire(6) < 1 and self.Train.DeadmanUF.IsPressed < 1 or self.Train:ReadTrainWire(6) < 1 and self.Train.DeadmanUF.IsPressed < 1
							and self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) then
						self.TractionConditionFulfilled = false
				elseif self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(12) < 1 or self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(12) < 1
							and self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) then
						self.TractionConditionFulfilled = false
				end
		...
		if self.TractionConditionFulfilled == true then
				...
				if self.Train:GetNW2Bool("DeadmanTripped", false) == false then
						...
						if self.VZ == true then
								...
								if self.Traction == 0 then self.Train:WriteTrainWire(1, self.Traction) end
...
function TRAIN_SYSTEM:U2Engine()
		...
		elseif self.ResistorBank == self.CurrentResistor then
				...
				if self.CamshaftFinishedMoving == true then self.CamshaftMoveTimer = 0 end
		end
...
function TRAIN_SYSTEM:MUHandler()
		...
		elseif self.ReverserLeverStateA == -1 and self.ReverserLeverStateB == 0 then
				if self.LeadingCabA == 1 then self.ReverserState = -1 end
				self.VE = true
				self.VZ = false
				self.Train:WriteTrainWire(6, 0)
		elseif self.ReverserLeverStateA == 1 and self.ReverserLeverStateB == 0 then
				if self.LeadingCabA == 1 then self.ReverserState = 0 end
