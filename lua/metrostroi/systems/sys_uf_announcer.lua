...
function TRAIN_SYSTEM:Inputs() return {"Reset"} end
...
if SERVER then
    ...
    function TRAIN_SYSTEM:TriggerInput(name, value)
        ...
        if name == "Table" then self.AnnTable = value end
    end
    ...
    function TRAIN_SYSTEM:Think()
        ...
        if #self.Schedule == 0 and self.AnnounceTimer and CurTime() -
            self.AnnounceTimer > 0 then
            self.AnnounceTimer = nil -- no need to run a timer when self.Schedule is null

        end
        if #self.Schedule > ANNOUNCER_CACHE_LIMIT then self:Reset() end
    end
else
    net.Receive("uf_announcer", function(len, pl)
        ...
        if train.AnnouncerPositions then
            ...
            for k, v in ipairs(train.AnnouncerPositions) do
                train:PlayOnceFromPos("announcer" .. k, snd, 0.4, 1, 400, 1e9,
                                      v[1])
            end
        else
            train:PlayOnceFromPos("announcer", snd, train.OnAnnouncer and
                                      train:OnAnnouncer(1) or 0.4, 1, 600, 1e9,
                                  Vector(0, 0, 0))
        end
    end)

    function TRAIN_SYSTEM:ClientInitialize() end

    function TRAIN_SYSTEM:ClientThink() end
end
