--------------------------------------------------------------------------------
-- 81-702 pneumatic system
--------------------------------------------------------------------------------
-- Copyright (C) 2013-2018 Metrostroi Team & FoxWorks Aerospace s.r.o.
-- Contains proprietary code. See license.txt for additional information.
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("b_wagen_Pneumatic")
...
function TRAIN_SYSTEM:Think(dT)
    ...
    self.PneumaticNo1Pressure_dPdT = 0.0
    self.PneumaticNo2Pressure_dPdT = 0.0
    ...
    if self.EmergencyValve then
        --local leakst = math.max(0,(pr_speed*1.17*(self.BrakeLinePressure-math.min(2.5,self.TrainToBrakeReducedPressure))*0.9))
    ...
    if Train.AirDistributorDisconnect.Value == 0 then
        ...
        if (Train.PneumaticNo1.Value == 1.0) and (Train.PneumaticNo2.Value == 0.0) then
            if self.PN1 == 0 then
                --1,2
                self.PN1 = math.min(self.TrainLinePressure,(1.2 + error + self.WeightLoadRatio*0.6))
                if self.BePN1 == false and self.BrakeCylinderPressure > 0.2 then
                    Train:PlayOnce("PN1end","stop")
                end
                self.BePN1 = true
            end
        ...
        if Train.PneumaticNo2.Value == 1.0 then
            if self.PN2 == 0 then
                self.PN2 = math.min(self.TrainLinePressure,(2.2 + error + self.WeightLoadRatio*1.2))
        ...
        if math.abs(self.BrakeCylinderPressure - targetPressure) > 0.32 then
        ...
        if self.BrakeCylinderValve == 1 then
            self:equalizePressure(dT,"BrakeCylinderPressure", math.min(2.7 + self.WeightLoadRatio*1.3,targetPressure), 1+math.Clamp((self.BrakeCylinderPressure-0.5)/2.8,0,0.7), 2, nil, 0.8+math.Clamp((self.BrakeCylinderPressure-0.75)/0.6,0,1)) --0.75, 1.25)
        end
