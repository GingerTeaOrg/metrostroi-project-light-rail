...
function TRAIN_SYSTEM:Think(dT)
	...
	self.SPAD = IsValid(self.PassedSignal) and (self.SignalTrackPos.forward and self.ScanLocation.x > self.SignalTrackPos.x)
				            or (not self.SignalTrackPos.forward and self.ScanLocation.x < self.SignalTrackPos.x) and self.ScanLocation.path == self.SignalTrackPos.path
	if self.SPAD then train:SetNW2Bool("DeadmanTripped", self.SPAD) end

	if self.SpeedLimitForward or self.SpeedLimitBackward then
		deadman:OverspeedProtection((direction and speed > self.SpeedLimitForward) or (not direction and speed > self.SpeedLimitBackward),
		                            direction and self.SpeedLimitForward or self.SpeedLimitBackward)
	end
end
