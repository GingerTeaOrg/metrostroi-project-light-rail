...
function TRAIN_SYSTEM:Think(Train)
    ...
    if self.Train:GetNW2Bool("BatteryOn", false) == true or
        self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(7) > 0 then -- if either the battery is on or the EMU cables signal multiple unit mode
        ...
        if self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(12) > 0 and
            self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) == false then
            self.TractionConditionFulfilled = true
        elseif self.Train:ReadTrainWire(6) < 1 and
            self.Train.DeadmanUF.IsPressed > 0 then
            self.TractionConditionFulfilled = true
        elseif self.Train:ReadTrainWire(6) < 1 and
            self.Train.DeadmanUF.IsPressed < 1 or self.Train:ReadTrainWire(6) <
            1 and self.Train.DeadmanUF.IsPressed < 1 and
            self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) then
            self.TractionConditionFulfilled = false
        elseif self.Train:ReadTrainWire(6) > 0 and self.Train:ReadTrainWire(12) <
            1 or self.Train:ReadTrainWire(6) > 0 and
            self.Train:ReadTrainWire(12) < 1 and
            self:CheckDoorsAllClosed(self.Train.CheckDoorsClosed) then
            self.TractionConditionFulfilled = false
        end
        ...
        elseif self.ReverserState == 0 then
            self.TractionConditionFulfilled = false
        elseif self.Train:ReadTrainWire(3) > 0 and self.Train:ReadTrainWire(6) >
            0 or self.Train:ReadTrainWire(4) > 0 then
            self.TractionConditionFulfilled = true
        end

        if self.ReverserState == 1 and self.Train.DeadmanUF.IsPressed == 0 and
            self.ThrottleState > 0 then
            self.Train:SetNW2Bool("TractionAppliedWhileStillNoDeadman", true)

        elseif self.ReverserState == -1 and self.Train.DeadmanUF.IsPressed == 0 and
            self.ThrottleState > 0 then
            self.Train:SetNW2Bool("TractionAppliedWhileStillNoDeadman", true)
        elseif self.ReverserState == -1 and self.Train.DeadmanUF.IsPressed == 0 and
            self.ThrottleState <= 0 then
            self.Train:SetNW2Bool("TractionAppliedWhileStillNoDeadman", false)
        elseif self.ReverserState == -1 and self.Train.DeadmanUF.IsPressed == 1 and
            self.ThrottleState > 0 then
            self.Train:SetNW2Bool("TractionAppliedWhileStillNoDeadman", false)
        elseif self.ReverserState == 1 and self.Train.DeadmanUF.IsPressed == 0 and
            self.ThrottleState <= 0 then
    ...
    elseif self.Train.Panel.BlinkerRight > 0 and self.Train.Panel.BlinkerLeft <
        1 then
        self.Train:SetNW2Float("BlinkerStatus", 0)
    elseif self.Train.Panel.BlinkerLeft < 1 and self.Train.Panel.BlinkerRight <
        1 then
        ...
        if self.Train:GetNW2Bool("DeadmanTripped", false) == false then
            ...
            if self.VZ == true then
                if self.Traction > 0 then
                    ...
                    self.Train:WriteTrainWire(1,
                                              math.Clamp(self.Traction, 0, 100))
                ...
                elseif self.Traction <= 0 then
                    ...
                    elseif self.Speed < 5 and self.Train:ReadTrainWire(2) > 0 and
                        self.Train:ReadTrainWire(5) == 2.7 then
                        self.Traction = 0
                        self.Train:WriteTrainWire(1, self.Traction)
                    elseif self.Speed > 5 and self.Train:ReadTrainWire(2) > 0 and
                        self.Train:ReadTrainWire(5) < 2.7 then
            ...
            elseif self.VE == true or self.ReverserLeverStateA == -1 or
                self.ReverserLeverStateB == -1 then
                if self.Traction > 0 then
                    self.Traction = self.Traction
                    self.BrakePressure = 0
                    -- print("ve sys")
            ...
            elseif self.ReverserLeverStateA == 3 or self.ReverserLeverStateB ==
                -1 then
                self.ReverserState = 1
            end
    ...
    if self.ThrottleState < 0 and self.Speed < 8 or self.Train:ReadTrainWire(10) >
        0 then -- Pneumatic Brakes engaged when electric brakes have brought down the speed to very low
...
function TRAIN_SYSTEM:IsLeadingCab()
    local output
    if self.ReverserInsertedA then
        output = "a"
    elseif self.ReverserInsertedB then
        output = "b"
    else
        output = "none"
    end
...
function TRAIN_SYSTEM:MUHandler()
    ...
    if self.ReverserLeverStateA == -1 and self.ReverserLeverStateB == 0 then
        if self:IsLeadingCab() == "a" then self.ReverserState = -1 end
        self.VE = true
        self.VZ = false
        self.Train:WriteTrainWire(6, 0)
    elseif self.ReverserLeverStateA == 1 and self.ReverserLeverStateB == 0 then
        if self:IsLeadingCab() == "a" then self.ReverserState = 0 end
    ...
    if self.Train:ReadTrainWire(6) > 0 then -- if we're not the leading train, read this stuff from train wires
        ...
        elseif self.Train:ReadTrainWire(3) < 1 and self.Train:ReadTrainWire(4) >
            0 then -- opposite means backwards
            self.ReverserState = -1
        elseif self.Train:ReadTrainWire(3) < 1 and self.Train:ReadTrainWire(4) <
            1 then
            self.ReverserState = 0
        end
    elseif self:IsLeadingCab() == "a" and self.Train:ReadTrainWire(6) < 1 then
        self.VZ = false
