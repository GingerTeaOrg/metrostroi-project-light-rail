...
function TRAIN_SYSTEM:Initialize()
    ...
    self.TriggerNames = {
        ...
        "Number5", -- 5
        "Number6", -- 6
        "Number7", "Number8", "Number9", "Number0", "Destination", -- 11
        "Delete", -- 12
        "Enter", -- 13
...
function TRAIN_SYSTEM:PrintText(x, y, text, inverse)
    ...
    for i = 1, #str do
        ...
        if inverse then
            draw.SimpleText(string.char(0x7f), "IBIS", (x + i) * 20.5 + 5,
                            y * 40 + 40, Color(0, 0, 0), TEXT_ALIGN_CENTER,
                            TEXT_ALIGN_CENTER)
            draw.SimpleText(char, "IBIS", (x + i) * 20.5 + 5, y * 40 + 40,
                            Color(140, 190, 0, 150), TEXT_ALIGN_CENTER,
                            TEXT_ALIGN_CENTER)
        else
            draw.SimpleText(char, "IBIS", (x + i) * 32, y * 15 + 20,
...
function TRAIN_SYSTEM:IBISScreen(Train)
    ...
    local Route = self.Train:GetNW2String("IBIS:RouteChar1") ..
                      self.Train:GetNW2String("IBIS:RouteChar2")
    local Course = self.Train:GetNW2String("IBIS:CourseChar1") ..
                       self.Train:GetNW2String("IBIS:CourseChar2") ..
                       self.Train:GetNW2String("IBIS:CourseChar3") ..
                       self.Train:GetNW2String("IBIS:CourseChar4")
    local Destination = self.Train:GetNW2String("IBIS:DestinationChar1") ..
                            self.Train:GetNW2String("IBIS:DestinationChar2") ..
                            self.Train:GetNW2String("IBIS:DestinationChar3")
    ...
    local ServiceAnnouncement = self.Train:GetNW2String(
                                    "IBIS:ServiceAnnouncement", " ")
    ----print(Route)
    ----print(Course)
    ...
    if State == 4 then
        self:BlinkText(true,
                       self.DefectString[self.Train:GetNW2Int("Defect", 1)])
    end
...
function TRAIN_SYSTEM:Think()
    ...
    if self.CurrentStation ~= -1 and self.CurrentStation ~= nil and
        ...
        self.Train:SetNW2String("CurrentStation",
                                self.DestinationTable[self.CurrentStation])
...
if SERVER then
    ...
    function TRAIN_SYSTEM:InputProcessor(Input)
        ...
        if self.State == 1 and self.Menu == 4 or self.Menu == 1 and self.State ==
            1 or self.State == 2 and self.Menu == 4 then -- only during startup in menus or manually triggered menus

            self:LinieKurs(self.KeyInput)

        elseif self.State == 1 and self.Menu == 5 or self.Menu == 2 and
            self.State == 1 or self.Menu == 5 and self.State == 2 then -- same procedure as above for routes as well
    ...
    function TRAIN_SYSTEM:LinieKurs(KeyInput) -- split Line/Course into a separate function
        ...
        if KeyInput == nil or KeyInput == "TimeAndDate" or KeyInput == "Enter" or
            KeyInput == "ServiceAnnouncements" then return end

        if KeyInput ~= "Delete" and KeyInput ~= nil and tonumber(KeyInput, 10) and
            not tonumber(self.CourseChar1, 10) then -- Input starts at the last digit and gets shifted around at every turn, until the whole prompt is full
    ...
    function TRAIN_SYSTEM:RouteInput(Input) -- split Route into a separate function
        ...
        if Input == nil or Input == "TimeAndDate" or Input == "Enter" or Input ==
            "ServiceAnnouncements" then return end
        if Input ~= nil and tonumber(Input, 10) and
            not tonumber(self.RouteChar1, 10) then -- Input starts at the last digit and gets shifted around at every turn, until the whole prompt is full
    ...
    function TRAIN_SYSTEM:DestinationInput(Input)
        if Input == nil or Input == "TimeAndDate" or Input == "Enter" or Input ==
            "ServiceAnnouncements" then return end
        if Input ~= nil and tonumber(Input, 10) and
            not tonumber(self.DestinationChar1, 10) then -- Input starts at the last digit and gets shifted around at every turn, until the whole prompt is full
            self.DestinationChar1 = self.DestinationChar2
...
function TRAIN_SYSTEM:CANReceive(SourceCar, target, targetdata)
    ...
    if target == "ServiceAnnouncement" then
        self.ServiceAnnouncement = targetdata
    end
...
function TRAIN_SYSTEM:CANWrite(target, targetdata)
    for i in self.Train.WagonList do
        ...
        if sys and sys.CANReceive then
            sys:CANReceive(self.Train, target, targetdata)
        end
    end
end
...
function TRAIN_SYSTEM:Switching(Left, Right)
    ...
    if CurTime() - self.Timer > 2.00 or self.TimerToggle then
        ...
        if pos then
            ...
            local light = Metrostroi.GetNextTrafficLight(pos.node1, pos.x,
                                                         pos.forward)
            local function getSignal(base, chan)
                if (chan == 1) and (base == "alt") and light and
                    light:GetInvertChannel1() then return "main" end
                if (chan == 2) and (base == "alt") and light and
                    light:GetInvertChannel2() then return "main" end
                return base
            end

            -- Get switches and trigger them all
            local switches = Metrostroi.GetTrackSwitches(pos.node1, pos.x,
                                                         pos.forward)
            for _, switch in pairs(switches) do
                Alt1 = Alt1 or
                           (switch:GetChannel() == 1 and switch:GetSignal() > 0)
                Alt2 = Alt2 or
                           (switch:GetChannel() == 2 and switch:GetSignal() > 0)
                no_switches = false
                if self.SelectAlternate == true then
                    ...
                    if self.Channel == 2 then
                        switch:SendSignal(getSignal("alt", 2), 2)
                    end
                elseif self.SelectAlternate == false then
                    ...
                    if self.Channel == 2 then
                        switch:SendSignal(getSignal("main", 2), 2)
                    end
                end
...
function TRAIN_SYSTEM:AutomaticAnnouncement()
    ...
    local pos1 = Metrostroi.GetPositionOnTrack(platform.PlatformStart,
                                               dir:Angle())[1]
    local pos2 =
        Metrostroi.GetPositionOnTrack(platform.PlatformEnd, dir:Angle())[1]
    ...
    local trainPos = Metrostroi.GetPositionOnTrack(self.Train:GetPos(),
                                                   self.Train:GetAngles())[1]
    ...
    if dir1 == "up" and enteredVia ~= "null" then
        if enteredVia == "start" and trainPos.x > pos2.x and trainPos.x - pos2.x >
            30 then
            self.CurrentStationInternal = self.CurrentStationInternal + 1
            self.CurrentStation = self.RouteTable[self.CourseChar1 ..
                                      self.CourseChar2][self.RouteChar1 ..
        ...
        elseif enteredVia == "end" and trainPos.x < pos1.x and trainPos.x +
            pos1.x > 30 then -- station points upwards on the coordinate system and we are travelling downwards the system, that means the start is our last point of contact from which we measure distance, DOWNwards the coordinate system
            self.CurrentStationInternal = self.CurrentStationInternal + 1
            self.CurrentStation = self.RouteTable[self.CourseChar1 ..
                                      self.CourseChar2][self.RouteChar1 ..
                                      self.RouteChar2][self.CurrentStationInternal]
    ...
    elseif dir1 == "up" and enteredVia == "null" and
        (trainPos.x < pos1.x or trainPos.x > pos2.x) then -- if the train is in a terminus or got spawned there, there is no detected entry point, so just take any exit as the point to measure from
        self.CurrentStationInternal = self.CurrentStationInternal + 1
        self.CurrentStation = self.RouteTable[self.CourseChar1 ..
                                  self.CourseChar2][self.RouteChar1 ..
                                  self.RouteChar2][self.CurrentStationInternal]
        enteredVia = "null"
    elseif dir2 == "down" and enteredVia == "null" and
        (trainPos.x < pos2.x or trainPos.x > pos1.x) then
        self.CurrentStationInternal = self.CurrentStationInternal + 1
        self.CurrentStation = self.RouteTable[self.CourseChar1 ..
                                  self.CourseChar2][self.RouteChar1 ..
                                  self.RouteChar2][self.CurrentStationInternal]
