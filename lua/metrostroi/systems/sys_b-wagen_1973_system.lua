...
function TRAIN_SYSTEM:Mirrors()
    ...
    local t = self.Train
    t:SetNW2Int( "MirrorLeft", p.MirrorLeft )
    t:SetNW2Int( "MirrorRight", p.MirrorRight )
end
...
function TRAIN_SYSTEM:BlinkerHandler()
    local t = self.Train
    local blinkerLeft = self.Panel.BlinkerLeft > 0 or t:ReadTrainWire( 20 ) > 0 or t:ReadTrainWire( 22 ) > 0
    local blinkerRight = self.Panel.BlinkerRight > 0 or t:ReadTrainWire( 21 ) > 0 or t:ReadTrainWire( 22 ) > 0
    ...
    local power = self.CircuitBreakerOn or t:ReadTrainWire( 6 ) > 0
...
function TRAIN_SYSTEM:Traction()
    local t = self.Train
    local tractionUnlocked = ( self.ReverserA > 1 or self.ReverserA < 0 ) or ( self.ReverserB > 1 or self.ReverserB < 0 ) or ( t:ReadTrainWire( 3 ) > 0 or t:ReadTrainWire( 4 ) > 0 )
